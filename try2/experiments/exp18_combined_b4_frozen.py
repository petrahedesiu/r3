...
class Config(ExperimentConfig):
    EXPERIMENT_NAME = "exp18_combined_b4_frozen"
    DESCRIPTION = "Combined data + EfficientNet-B4 + frozen encoder (5 epochs) + compound loss + strong augmentation"
...
def load_data():
    ...
    for data_dir in DATA_DIRS:
        if os.path.isdir(data_dir):
            ...
            print(f"  Found {len(pts)} patients in {data_dir}")
        else:
            print(f"  WARNING: Directory not found: {data_dir}")

    volumes, segmentations = [], []
    for p in tqdm(all_patients, desc="Loading patients"):
        try:
...
def main():
    ...
    print(f"Model parameters (before freeze): {num_params:,}")

    freeze_encoder(model)
    # count what is left trainable
    num_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Encoder FROZEN. Trainable parameters: {num_trainable:,}")
    ...
    logger.info(f"Freeze encoder    : first {FREEZE_ENCODER_EPOCHS} epochs")
    ...
    print("\n" + "=" * 70)
    print("EXP18: COMBINED DATA + FROZEN ENCODER TRAINING")
    print("=" * 70)
    ...
    for epoch in range(1, cfg.NUM_EPOCHS + 1):
        ...
        if epoch == FREEZE_ENCODER_EPOCHS + 1:
            ...
            num_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
            logger.info(f"Encoder UNFROZEN at epoch {epoch} with LR={cfg.LR*0.1}")
            logger.info(f"Trainable parameters after unfreeze: {num_trainable:,}")
