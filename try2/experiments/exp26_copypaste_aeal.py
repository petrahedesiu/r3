...
from shared.dataset import _normalize, _roi_crop, _to_tensors
...
class Config(ExperimentConfig):
    EXPERIMENT_NAME = "exp26_copypaste_aeal"
    DESCRIPTION = "Copy-paste AEAL specialist with anti-overfitting interventions"
...
class CopyPasteFGCenteredDataset(Dataset):
    ...
    def __init__(
        ...
        for patient_idx, seg in enumerate(segmentations):
            ...
            for slice_idx in range(n_slices):
                ...
                if s.max() > 0:
                    ys, xs = np.where(s > 0)
                    self.fg_pixels[(patient_idx, slice_idx)] = np.stack([ys, xs], axis=1)
    ...
    def _crop_patch_around(
        ...
        ps = self.patch_size
        jit = ps // 8
        cy = center_y + random.randint(-jit, jit)
        cx = center_x + random.randint(-jit, jit)
        H, W = image.shape[:2]
        y0 = cy - ps//2
        x0 = cx - ps//2
        y1 = y0 + ps
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        use_fg_center = (random.random() < self.fg_ratio and key in self.fg_pixels)
...
def load_data(data_dir):
    patient_list = discover_patients(data_dir)
...
def main():
    ...
    train_dataset = CopyPasteFGCenteredDataset(
    ...
    logger.info(f"FG ratio           : {cfg.FG_RATIO}")
    ...
    print("\n" + "=" * 70)
    print("TRAINING: COPY-PASTE AEAL SPECIALIST")
    print("=" * 70)
    ...
    print(f"  E2E AEAL Dice: {all_metrics['dice_per_class'].get(1, 0):.4f}")
    print(f"  vs exp13 baseline AEAL Dice: 0.703")
