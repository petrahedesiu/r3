...
class Config(ExperimentConfig):
    EXPERIMENT_NAME = "exp17_centroid_roi"
    DESCRIPTION = "Stage 1 localizer + exp13 fine model with ROI-style crop (no retraining)"
...
def main():
    ...
    if not fine_dir:
        fine_dir = find_latest_model_dir(results_base, "exp13_full_pipeline")
    if fine_dir is None:
        print("ERROR: No trained fine model found. Run exp13 first.")
        print(f"  Searched in: {results_base}/exp13_full_pipeline/")
        return
