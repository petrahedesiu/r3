...
class Config(ExperimentConfig):
    EXPERIMENT_NAME = "exp32_combined_data"
    DESCRIPTION = "Combined 52-patient pipeline: coarse + AEAL + AEAR, 2x data"

    DATA_DIRS = [
        os.path.join(ExperimentConfig.DATA_DIR, ""),
        os.path.join(os.path.dirname(ExperimentConfig.DATA_DIR), "CROP - februarie 2026"),
    ]
...
def load_data_combined(data_dirs):
    ...
    for d in data_dirs:
        if not os.path.isdir(d):
            ...
            continue
        pats = discover_patients(d)
        for p in tqdm(pats, desc=f"Loading from {os.path.basename(d)}"):
            try:
...
def train_epoch_coarse(model, dataloader, criterion, optimizer, device, epoch=0, num_classes=2):
    ...
    pbar = tqdm(dataloader, desc=f"  Coarse train ep {epoch}")
...
def validate_coarse(model, dataloader, criterion, device, epoch=0, num_classes=2):
    ...
    with torch.no_grad():
        for images, masks in tqdm(dataloader, desc=f"  Coarse val ep {epoch}"):
            images = images.to(device).float()
            masks = masks.to(device).long()
            outputs = model(images)
            loss, _ = criterion(outputs, masks, epoch=epoch)
...
def train_epoch_fine(model, dataloader, criterion, optimizer, device, epoch=0, num_classes=3):
    ...
    pbar = tqdm(dataloader, desc=f"  Fine train ep {epoch}")
    for images, masks in pbar:
        images = images.to(device).float()
        masks = masks.to(device).long()
...
def validate_fine(model, dataloader, criterion, device, epoch=0, num_classes=3):
    ...
    with torch.no_grad():
        for images, masks in tqdm(dataloader, desc=f"  Fine val ep {epoch}"):
            images = images.to(device).float()
            masks = masks.to(device).long()
...
def run_coarse_stage(image_norm, coarse_model, device, coarse_size=256, coarse_threshold=0.3):
...
def run_aeal_bbox_path(image_norm, coarse_binary, aeal_model, device,
    ...
    with torch.no_grad():
        fine_logits = aeal_model(fine_tensor)
...
def run_aear_patch_path(image_norm, coarse_binary, aear_model, device,
    ...
    with torch.no_grad():
        fine_logits = aear_model(fine_tensor)
        fine_pred = fine_logits.argmax(dim=1)[0].cpu().numpy()
...
def ensemble_predict_slice(image, coarse_model, aeal_model, aear_model, device,
...
def plot_ensemble_visualization(image, gt_mask, prediction, info, save_path, title=""):
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    axes[0].imshow(image, cmap="gray"); axes[0].set_title("CT Slice"); axes[0].axis("off")
    axes[1].imshow(gt_mask, cmap="tab10", vmin=0, vmax=2); axes[1].set_title("Ground Truth"); axes[1].axis("off")
    detected = info.get("detected", False)
    axes[2].imshow(prediction, cmap="tab10", vmin=0, vmax=2)
    axes[2].set_title(f"Prediction ({'detected' if detected else 'missed'})")
    axes[2].axis("off")
    if title:
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler,
                device, output_dir, model_name, num_epochs, num_classes,
    ...
    for epoch in range(1, num_epochs + 1):
        logger.info(f"  [{model_name}] Epoch {epoch}/{num_epochs}")
        ...
        logger.info(
            ...
            f"dice={train_metrics['dice']:.4f} recall={train_metrics['recall']:.4f}"
        )
        logger.info(
            f"    VAL    loss={val_metrics['loss']:.4f} "
            f"dice={val_metrics['dice']:.4f} recall={val_metrics['recall']:.4f} "
        ...
        if val_metrics["dice"] > best_dice:
            ...
            logger.info(f"    >>> NEW BEST {model_name} dice={val_metrics['dice']:.4f}")
    ...
    return history, best_epoch, best_dice
...
def main():
    ...
    print("\nLoading combined data from 2 directories...")
    data_dirs = [
        cfg.DATA_DIR,
    ...
    logger.info(f"Loaded {len(volumes)} patients from {len(data_dirs)} directories")
    ...
    logger.info(f"Train: {len(train_volumes)} patients, Val: {len(val_volumes)} patients")

    fg_train = sum(sum(1 for sl in range(s.shape[2]) if s[:, :, sl].max() > 0) for s in train_segs)
    fg_val = sum(sum(1 for sl in range(s.shape[2]) if s[:, :, sl].max() > 0) for s in val_segs)
    print(f"FG slices: train={fg_train}, val={fg_val}")
    logger.info(f"FG slices: train={fg_train}, val={fg_val}")

    device = torch.device(cfg.DEVICE)

    print("\n" + "=" * 70)
    print("PHASE 1: COARSE MODEL (binary, 256x256, UNet+B0)")
    print("=" * 70)
    logger.info("=" * 70)
    logger.info("PHASE 1: COARSE MODEL")

    coarse_class_weights = compute_binary_class_weights(train_segs, device)
    logger.info(f"Coarse class weights: {coarse_class_weights}")

    coarse_train_transform = get_transforms(train=True, img_size=cfg.COARSE_IMG_SIZE)
    coarse_val_transform = get_transforms(train=False, img_size=cfg.COARSE_IMG_SIZE)

    coarse_train_ds = CoarseFullSliceDataset(
        train_volumes, train_segs, transform=coarse_train_transform,
        oversample=cfg.COARSE_OVERSAMPLE,
    )
    coarse_val_ds = CoarseFullSliceDataset(
        val_volumes, val_segs, transform=coarse_val_transform, oversample=1,
    )
    coarse_train_loader = DataLoader(
        coarse_train_ds, batch_size=cfg.COARSE_BATCH_SIZE,
        shuffle=True, num_workers=cfg.NUM_WORKERS, pin_memory=True,
    )
    coarse_val_loader = DataLoader(
        coarse_val_ds, batch_size=cfg.COARSE_BATCH_SIZE,
        shuffle=False, num_workers=cfg.NUM_WORKERS, pin_memory=True,
    )
    print(f"Coarse: {len(coarse_train_ds)} train, {len(coarse_val_ds)} val samples")
    ...
    coarse_optimizer = torch.optim.AdamW(coarse_model.parameters(), lr=cfg.COARSE_LR, weight_decay=cfg.WEIGHT_DECAY)
    coarse_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        coarse_optimizer, T_0=cfg.SCHEDULER_T0, T_mult=cfg.SCHEDULER_TMULT, eta_min=cfg.SCHEDULER_ETA_MIN,
    )

    coarse_history, coarse_best_epoch, coarse_best_dice = train_model(
        coarse_model, coarse_train_loader, coarse_val_loader,
        coarse_criterion, coarse_optimizer, coarse_scheduler,
        device, output_dir, "coarse",
        num_epochs=cfg.NUM_EPOCHS, num_classes=2,
        train_fn=train_epoch_coarse, val_fn=validate_coarse,
        logger=logger,
    )

    del coarse_train_ds, coarse_val_ds, coarse_train_loader, coarse_val_loader
    del coarse_criterion, coarse_optimizer, coarse_scheduler
    if device.type == "mps":
        torch.mps.empty_cache()
    gc.collect()

    print("\n" + "=" * 70)
    print("PHASE 2: AEAL MODEL (3-class, 384x384, UNet+++B4+SCSE, FGCentered)")
    print("=" * 70)
    logger.info("=" * 70)
    logger.info("PHASE 2: AEAL MODEL")

    aeal_class_weights = compute_class_weights(train_segs, num_classes=3)
    logger.info(f"AEAL class weights: {aeal_class_weights}")

    aeal_train_transform = get_transforms(train=True, img_size=cfg.AEAL_IMG_SIZE)
    aeal_val_transform = get_transforms(train=False, img_size=cfg.AEAL_IMG_SIZE)

    aeal_train_ds = FGCenteredDataset(
        train_volumes, train_segs, transform=aeal_train_transform,
        fg_ratio=cfg.AEAL_FG_RATIO, patch_size=cfg.AEAL_IMG_SIZE,
        oversample=cfg.OVERSAMPLE_FACTOR,
    )
    aeal_val_ds = StandardDataset(
        val_volumes, val_segs, transform=aeal_val_transform, oversample=1,
    )
    aeal_train_loader = DataLoader(
        aeal_train_ds, batch_size=cfg.AEAL_BATCH_SIZE,
        shuffle=True, num_workers=cfg.NUM_WORKERS, pin_memory=True,
    )
    aeal_val_loader = DataLoader(
        aeal_val_ds, batch_size=cfg.AEAL_BATCH_SIZE,
        shuffle=False, num_workers=cfg.NUM_WORKERS, pin_memory=True,
    )
    print(f"AEAL: {len(aeal_train_ds)} train, {len(aeal_val_ds)} val samples")
    ...
    aeal_criterion = CompoundLoss(
        ...
        class_weights=aeal_class_weights.to(device),
        tversky_alpha=cfg.TVERSKY_ALPHA, tversky_beta=cfg.TVERSKY_BETA,
        focal_alpha=cfg.FOCAL_ALPHA, focal_gamma=cfg.FOCAL_GAMMA,
        use_boundary=True, epoch_for_boundary_rampup=cfg.AEAL_BOUNDARY_RAMPUP,
        num_classes=3,
    ).to(device)
    aeal_optimizer = torch.optim.AdamW(aeal_model.parameters(), lr=cfg.AEAL_LR, weight_decay=cfg.WEIGHT_DECAY)
    aeal_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        aeal_optimizer, T_0=cfg.SCHEDULER_T0, T_mult=cfg.SCHEDULER_TMULT, eta_min=cfg.SCHEDULER_ETA_MIN,
    )

    aeal_history, aeal_best_epoch, aeal_best_dice = train_model(
        aeal_model, aeal_train_loader, aeal_val_loader,
        aeal_criterion, aeal_optimizer, aeal_scheduler,
        device, output_dir, "aeal",
        num_epochs=cfg.NUM_EPOCHS, num_classes=3,
        train_fn=train_epoch_fine, val_fn=validate_fine,
        logger=logger,
    )

    del aeal_train_ds, aeal_val_ds, aeal_train_loader, aeal_val_loader
    del aeal_criterion, aeal_optimizer, aeal_scheduler
    if device.type == "mps":
        torch.mps.empty_cache()
    gc.collect()

    print("\n" + "=" * 70)
    print("PHASE 3: AEAR MODEL (3-class, 384x384, UNet+++B4+SCSE, patches)")
    print("=" * 70)
    logger.info("=" * 70)
    logger.info("PHASE 3: AEAR MODEL")

    aear_train_transform = get_transforms(train=True, img_size=cfg.AEAR_IMG_SIZE)
    aear_val_transform = get_transforms(train=False, img_size=cfg.AEAR_IMG_SIZE)

    aear_train_ds = FinePatchDataset(
        train_volumes, train_segs, transform=aear_train_transform,
        patch_size=cfg.AEAR_PATCH_SIZE, jitter=cfg.AEAR_JITTER_TRAIN,
        oversample=cfg.OVERSAMPLE_FACTOR,
    )
    aear_val_ds = FinePatchDataset(
        val_volumes, val_segs, transform=aear_val_transform,
        patch_size=cfg.AEAR_PATCH_SIZE, jitter=cfg.AEAR_JITTER_VAL,
        oversample=1,
    )
    aear_train_loader = DataLoader(
        aear_train_ds, batch_size=cfg.AEAR_BATCH_SIZE,
        shuffle=True, num_workers=cfg.NUM_WORKERS,
        pin_memory=(cfg.DEVICE != "mps"),
    )
    aear_val_loader = DataLoader(
        aear_val_ds, batch_size=cfg.AEAR_BATCH_SIZE,
        shuffle=False, num_workers=cfg.NUM_WORKERS,
        pin_memory=(cfg.DEVICE != "mps"),
    )
    print(f"AEAR: {len(aear_train_ds)} train, {len(aear_val_ds)} val samples")
    ...
    aear_optimizer = torch.optim.AdamW(aear_model.parameters(), lr=cfg.AEAR_LR, weight_decay=cfg.WEIGHT_DECAY)
    aear_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        aear_optimizer, T_0=cfg.SCHEDULER_T0, T_mult=cfg.SCHEDULER_TMULT, eta_min=cfg.SCHEDULER_ETA_MIN,
    )

    aear_history, aear_best_epoch, aear_best_dice = train_model(
        aear_model, aear_train_loader, aear_val_loader,
        aear_criterion, aear_optimizer, aear_scheduler,
        device, output_dir, "aear",
        num_epochs=cfg.NUM_EPOCHS, num_classes=3,
        train_fn=train_epoch_fine, val_fn=validate_fine,
        logger=logger,
    )
    ...
    del aear_criterion, aear_optimizer, aear_scheduler
    if device.type == "mps":
        torch.mps.empty_cache()
    gc.collect()

    print("\n" + "=" * 70)
    print("PHASE 4: END-TO-END EVALUATION")
    print("=" * 70)
    logger.info("=" * 70)
    logger.info("PHASE 4: E2E EVALUATION")

    coarse_ckpt = torch.load(output_dir / "coarse" / "best_model.pth", map_location=device, weights_only=False)
    coarse_model.load_state_dict(coarse_ckpt["model_state_dict"])
    coarse_model.eval()

    aeal_ckpt = torch.load(output_dir / "aeal" / "best_model.pth", map_location=device, weights_only=False)
    aeal_model.load_state_dict(aeal_ckpt["model_state_dict"])
    aeal_model.eval()

    aear_ckpt = torch.load(output_dir / "aear" / "best_model.pth", map_location=device, weights_only=False)
    aear_model.load_state_dict(aear_ckpt["model_state_dict"])
    aear_model.eval()

    logger.info("Loaded best models for E2E evaluation")
    ...
    logger.info(f"Stage 1 detection: {100*detection_rate:.1f}%")
    ...
    print(f"  exp32 E2E Dice: {all_metrics['mean_fg_dice']:.4f} ({delta:+.4f})")
    ...
    results = {
        ...
        "train_patients": len(train_volumes),
        "val_patients": len(val_volumes),
        "fg_train_slices": fg_train,
        "fg_val_slices": fg_val,

        "coarse": {
            "best_epoch": coarse_best_epoch,
            "best_dice": coarse_best_dice,
            "history": coarse_history,
        },
        "aeal": {
            "best_epoch": aeal_best_epoch,
            "best_dice": aeal_best_dice,
            "history": aeal_history,
        },
        "aear": {
            "best_epoch": aear_best_epoch,
            "best_dice": aear_best_dice,
            "history": aear_history,
        },
        ...
        "comparison": {
            ...
            "exp32_dice": all_metrics['mean_fg_dice'],
            "delta": delta,
        },
    ...
    print(f"  Total patients: {len(volumes)} ({len(train_volumes)} train, {len(val_volumes)} val)")
    print(f"  Coarse: best epoch {coarse_best_epoch}, dice {coarse_best_dice:.4f}")
    print(f"  AEAL:   best epoch {aeal_best_epoch}, dice {aeal_best_dice:.4f}")
    print(f"  AEAR:   best epoch {aear_best_epoch}, dice {aear_best_dice:.4f}")
    print(f"  E2E Dice (all):  {all_metrics['mean_fg_dice']:.4f}")
    print(f"  E2E Dice (FG):   {fg_metrics['mean_fg_dice']:.4f}")
    print(f"  AEAL Dice: {all_metrics['dice_per_class'][1]:.4f}")
    print(f"  AEAR Dice: {all_metrics['dice_per_class'][2]:.4f}")
