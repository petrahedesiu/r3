...
from shared.losses import TverskyLoss
...
class Config(ExperimentConfig):
    EXPERIMENT_NAME = "exp20_combined_b0_frozen"
    DESCRIPTION = "Combined data + EfficientNet-B0 + frozen encoder (5 epochs) + Tversky loss + strong augmentation"
...
def load_data():
    ...
    for data_dir in DATA_DIRS:
        ...
        print(f"  Found {len(pats)} patients in {data_dir}")
        for p in tqdm(pats, desc=f"Loading from {os.path.basename(data_dir)}"):
            try:
    ...
    return all_volumes, all_segmentations
...
def main():
    ...
    # print param counts
    print(f"Model parameters: {num_params_total:,} total, {num_params_trainable:,} trainable (encoder frozen)")

    criterion = TverskyLoss(alpha=cfg.TVERSKY_ALPHA, beta=cfg.TVERSKY_BETA)
    ...
    logger.info(f"Loss              : TverskyLoss only")
    logger.info(f"Encoder           : efficientnet-b0 (frozen first {FREEZE_ENCODER_EPOCHS} epochs)")
    ...
    print("\n" + "=" * 70)
    print("TRAINING: Combined B0 Frozen Encoder")
    print("=" * 70)
    ...
    for epoch in range(1, cfg.NUM_EPOCHS + 1):
        ...
        if epoch == FREEZE_ENCODER_EPOCHS + 1:
            logger.info(">>> UNFREEZING encoder -- switching to differential LR")
            ...
            num_params_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
            logger.info(f"  Trainable parameters after unfreeze: {num_params_trainable:,}")
        ...
        if val_metrics["dice"] > best_dice:
            ...
            logger.info(f"  No improvement for {epochs_without_improvement}/{cfg.EARLY_STOPPING_PATIENCE} epochs")
