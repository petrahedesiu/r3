...
class Config(ExperimentConfig):
    EXPERIMENT_NAME = "exp17_combined_b0_simple"
    DESCRIPTION = "Combined data + EfficientNet-B0 + Tversky-only loss + strong augmentation"
...
def load_data():
    ...
    for data_dir in DATA_DIRS:
        ...
        if os.path.isdir(resolved):
            pts = discover_patients(resolved)
            all_patients.extend(pts)
            print(f"  Found {len(pts)} patients in {resolved}")
        else:
            print(f"  WARNING: directory not found: {resolved}")
    print(f"  Total discovered patients: {len(all_patients)}")
...
def main():
    ...
    logger.info(f"Loss              : TverskyLoss only (no Focal/Lovasz/Boundary)")
    ...
    patience_counter = 0
    print("\n" + "=" * 70)
    print("TRAINING: COMBINED DATA + B0 + TVERSKY-ONLY")
    print("=" * 70)
