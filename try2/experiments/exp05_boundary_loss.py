...
from shared.training import plot_training_history, plot_predictions
...
class Config(ExperimentConfig):
    EXPERIMENT_NAME = "exp05_boundary_loss"
    DESCRIPTION = "Add boundary loss with linear ramp-up over 15 epochs"
...
def train_epoch_boundary(
    ...
    for images, masks in pbar:
        ...
        if "boundary" in loss_dict:
            bnd_str = f" bnd={loss_dict['boundary']:.3f}"
            eff_w = loss_dict.get("boundary_weight_effective", 0.0)
            bnd_str += f"(w={eff_w:.3f})"
...
def main():
    ...
    logger.info(f"Boundary weight (max): {cfg.BOUNDARY_WEIGHT}")
    logger.info(f"Boundary ramp-up epochs: {cfg.EPOCH_FOR_BOUNDARY_RAMPUP}")
